...
function Dalbum(f, m, w, h, o, s) {
	...
	c += '<form method="post" target="UploadAlbum" id="UploadAlbum" enctype="multipart/form-data" action="http://ceshi.hdjincheng.cn/ShangPu/ImageUpload"><input type="hidden" name="fid" value="'+f+'"/><input type="hidden" name="moduleid" value="'+m+'"/><input type="hidden" name="from" value="album"/><input type="hidden" name="old" value="'+o+'"/><table cellpadding="6"><tr><td><input id="remote_0" type="radio" name="isremote" value="0" checked onclick="ReLo(0, \'upalbum\');"/><label for="remote_0"> '+L['up_local']+'</label>&nbsp;&nbsp;&nbsp;<input id="remote_1" type="radio" name="isremote" value="1" onclick="ReLo(1, \'upalbum\');"/><label for="remote_1"> '+L['up_remote']+'</label></td></tr><tr id="remote_url" style="display:none;"><td><input id="remote" type="text" size="28" name="remote" value="http://" onclick="if(this.value==\'http://\')this.value=\'\';"/></td></tr><tr id="local_url"><td><input id="upalbum" type="file" size="20" name="upalbum" accept="image/*" onchange="if(isImg(\'upalbum\')){$("#UploadAlbum").submit();Dd(\'Dsubmit\').disabled=true;Dd(\'Dsubmit\').value=\''+L['uploading']+'\';}"/></td></tr><tr style="display:'+s+'"><td>'+L['width']+' <input type="text" size="3" name="width" value="'+w+'"/> px &nbsp;&nbsp;&nbsp;'+L['height']+' <input type="text" size="3" name="height" value="'+h+'"/> px </td></tr><tr><td><input type="submit" class="btn-b" value="'+L['upload']+'" id="Dsubmit"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="btn" value="'+L['cancel']+'" onclick="cDialog();"/></td></tr></table></form>';
	mkDialog('', c, L['upload_img'], 250);

	$('#UploadAlbum').ajaxForm(function(data){
			// alert(456)
			console.log(JSON.parse(data).imageName);
		$('#showthumb'+f).attr('src','http://ceshi.hdjincheng.cn/ShangPu/upload/'+JSON.parse(data).imageName);
        $('#showthumb'+f).attr('title',JSON.parse(data).imageName);
	})
}
function Dphoto(f, m, w, h, o, s) {
	var s = s ? 'none' : ''; var c = '<iframe id="UploadPhoto" name="UploadPhoto" style="display:none" src=""></iframe>';
	c += '<form method="post" target="UploadPhoto" enctype="multipart/form-data" action="http://ceshi.hdjincheng.cn/ShangPu/ImageUpload" onsubmit="return isUP(\'upalbum\');"><input type="hidden" name="fid" value="'+f+'"/><input type="hidden" name="moduleid" value="'+m+'"/><input type="hidden" name="from" value="photo"/><input type="hidden" name="old" value="'+o+'"/><table cellpadding="6"><tr><td><input id="remote_0" type="radio" name="isremote" value="0" checked onclick="ReLo(0, \'upalbum\');"/><label for="remote_0"> '+L['up_local']+'</label>&nbsp;&nbsp;&nbsp;<input id="remote_1" type="radio" name="isremote" value="1" onclick="ReLo(1, \'upalbum\');"/><label for="remote_1"> '+L['up_remote']+'</label></td></tr><tr id="remote_url" style="display:none;"><td><input id="remote" type="text" size="28" name="remote" value="http://" onclick="if(this.value==\'http://\')this.value=\'\';"/></td></tr><tr id="local_url"><td><input id="upalbum" type="file" size="20" name="upalbum" accept="image/*" onchange="if(isImg(\'upalbum\')){this.form.submit();Dd(\'Dsubmit\').disabled=true;Dd(\'Dsubmit\').value=\''+L['uploading']+'\';}"/></td></tr><tr style="display:'+s+'"><td>'+L['width']+' <input type="text" size="3" name="width" value="'+w+'"/> px &nbsp;&nbsp;&nbsp;'+L['height']+' <input type="text" size="3" name="height" value="'+h+'"/> px </td></tr><tr><td><input type="submit" class="btn-b" value="'+L['upload']+'" id="Dsubmit"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="btn" value="'+L['cancel']+'" onclick="cDialog();"/></td></tr></table></form>';
	mkDialog('', c, L['upload_img'], 250);
}
...
function isUP(i) {
	if(Dd('remote_0').checked) {
		...
		if(Dd('remote').value.length < 18) {
			...
			Dd('Dsubmit').disabled=true;
			Dd('Dsubmit').value=L['uploading'];
		}
	}
}
